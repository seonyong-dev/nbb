...
[📝일별 개발일지 (Notion)](https://chip-fifth-153.notion.site/NBB-Personal-Project-3836564d4062807a8406e903c55d7992?source=copy_link" target="_blank" rel="noopener noreferrer)

<br/><br/>

## 📅기간

- **2026.05.25 ~ 진행 중 (현재 FE-BE 연동 단계)**
...
5. <a href="#trouble">트러블 슈팅</a>
6. <a href="#next">다음 개발 목표</a>
...
데이터 무결성과 보안을 고려하여 설계한 그룹 정산 및 가상 뱅킹 서비스
...
<a name="trouble"></a>
<details>
<summary>렌더링 무한루프 해결</summary>

- **문제:** 화면 이동 시 무한 루프 발생.
- **원인:** useGroupInside의 useEffect 의존성 배열 미지정.
- **해결:** 빈 배열([])을 추가하여 최초 렌더링 시에만 실행되도록 수정.

</details>

<details>
<summary>서버-DB 연결 실패 해결</summary>

- **문제 1:** 서버-DB 연결 실패.
    - **원인:** Supabase Direct 연결의 IPv6 미지원 이슈.
    - **해결:** Session Pooler 주소로 변경하여 해결.
- **문제 2:** JPA 규칙 위반 및 Fetch Join 오류.
    - **해결:** 오류 코드 분석을 통한 JPA 규칙 재숙지 및 쿼리 수정.

</details>

<details>
<summary>로컬 개발 환경에서의 CORS 에러 발생 및 해결</summary>

- **문제:** 프론트엔드(Vite, Port 5173)에서 백엔드(Port 8080) API 호출 시 브라우저에서 CORS 정책 위반 에러 발생.
- **원인:** 브라우저의 동일 출처 정책(SOP)으로 인해 프로토콜, 도메인, 포트 중 하나라도 다르면 리소스 요청이 차단됨.
- **해결:** `vite.config.js` 파일에 `proxy` 설정을 추가하여 프론트엔드에서 나가는 API 요청을 백엔드 서버로 우회시켜 CORS 이슈를 해결함.
...
- 인증/인가 공통 로직에 대한 고민 및 적용
- 생략한 기능(회원가입 등) 구현 및 초기 계획한 서비스 확장
